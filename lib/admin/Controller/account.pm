...
sub detail : Local {
    ...
    if($c->config->{billing_features}) {
        ...
        if(ref $c->session->{restore_account_input} eq 'HASH') {
            $$voip_account{product} = $c->session->{restore_account_input}{product};
            $$voip_account{billing_profile} = $c->session->{restore_account_input}{billing_profile};
            $$voip_account{customer_id} = $c->session->{restore_account_input}{customer_id};
